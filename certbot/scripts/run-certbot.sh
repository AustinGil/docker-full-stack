#!/bin/bash

# certbot certonly --webroot -w /etc/letsencrypt -d "$DOMAIN" -d "www.$DOMAIN" --agree-tos --email "$EMAIL" --non-interactive --text

certbot certonly --webroot -w /data/letsencrypt --agree-tos --register-unsafely-without-email --non-interactive --text -d "$DOMAIN" --force-renewal # Add --staging flag for testing purposes

# Create directory with domain name within shared volume
mkdir -p /data/certs/"$DOMAIN"
# Move cert files to shared volume at folder name
cp /etc/letsencrypt/live/"$DOMAIN"/{cert.pem,privkey.pem} "$_"
